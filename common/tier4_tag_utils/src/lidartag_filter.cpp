...
: Node("extrinsic_tag_based_calibrator_node", options)
{
  ...
  process_noise_transl_dot_ = this->declare_parameter<double>("process_noise_transl_dot");
  ...
  const lidartag_msgs::msg::LidarTagDetection & detection, const std_msgs::msg::Header & header)
{
  ...
  if (hypotheses_map_.count(detection.id) == 0) {
    ...
    h.setMeasurementNoise(measurement_noise_transl_, measurement_noise_rot_);
    h.setNewHypothesisThreshold(new_hypothesis_transl_, new_hypothesis_rot_);
    h.setProcessNoise(
      process_noise_transl_, process_noise_transl_dot_, process_noise_rot_, process_noise_rot_dot_);
