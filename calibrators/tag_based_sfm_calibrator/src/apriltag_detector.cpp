  ...
  cy_(-1)
{
  for (const auto & tag_parameters : tag_parameters) {
    ...
    if (apriltag_family_map.count(family_name) != 0) {
      continue;
    }

    if (tag_create_fn_map.count(family_name) != 1) {
      RCLCPP_FATAL(
        rclcpp::get_logger("apriltag_detector"), "The tag family %s is invalid",
        family_name.c_str());
      continue;
    }
...
GroupedApriltagGridDetections ApriltagDetector::detect(const cv::Mat & cv_img) const
{
  ...
  zarray_t * detections =
    apriltag_detector_detect(apriltag_detector_, &apriltag_img);  // cSpell:ignore zarray
  ...
  for (int i = 0; i < zarray_size(detections); i++) {
    ...
    // Extra filter since the detector finds false positives with a high margin for out-of-plane
    // rotations
    ...
    if (fx_ > 0.0 && fy_ > 0.0 && cx_ > 0.0 && cy_ > 0.0) {
      ...
      detection_info.tagsize = tag_size;  // cSpell:ignore tagsize
      ...
      if (std::abs(cv::determinant(rotation) - 1.0) > 1e-5) {
        RCLCPP_WARN(
          rclcpp::get_logger("apriltag_detector"),
          "Detected apriltag: %s but discarded due to its rotation not having unit determinant\t "
          "det=%.2f",
      ...
      matd_destroy(pose.R);  // cSpell:ignore matd
      matd_destroy(pose.t);
    }
    ...
    if (
      ...
      RCLCPP_WARN(
        rclcpp::get_logger("apriltag_detector"),
        "Detected apriltag: %s but discarded due to its reprojection error\t margin: %.2f\t "
        "hom.error=%.2f\t reprojection.error=%.2f out_angle=%.2f deg",
    ...
    if (
      ...
      RCLCPP_WARN(
        rclcpp::get_logger("apriltag_detector"),
        "Detected apriltag: %s but discarded due to its out-of-plane angle\t margin: %.2f\t "
        "hom.error=%.2f\t reprojection.error=%.2f out_angle=%.2f deg",
        tag_family_and_id.c_str(), det->decision_margin, max_homography_error, reprojection_error,
        rotation_angle);
      continue;
    ...
    RCLCPP_INFO(
      rclcpp::get_logger("apriltag_detector"),
      "Detected apriltag: %s \t margin: %.2f\t hom.error=%.2f\t reprojection.error=%.2f "
      "out_angle=%.2f deg",
      tag_family_and_id.c_str(), det->decision_margin, max_homography_error, reprojection_error,
      rotation_angle);
