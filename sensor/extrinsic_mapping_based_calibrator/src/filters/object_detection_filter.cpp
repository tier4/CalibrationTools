...
{
  if (mapping_data->detected_objects_.size() == 0) {
  ...
  try {
    ...
    geometry_msgs::msg::Transform mapping_lidar_to_detection_frame_msg_ =
      tf_buffer_->lookupTransform(mapping_data->mapping_lidar_frame_, mapping_data->detected_objects_.front().header_.frame_id, t, timeout)
        .transform;

    geometry_msgs::msg::Transform source_lidar_to_mapping_lidar_msg_ =
      tf_buffer_->lookupTransform(calibration_frames.front().source_header_.frame_id, mapping_data->mapping_lidar_frame_, t, timeout)
        .transform;

    mapping_lidar_to_detection_frame_transform = tf2::transformToEigen(mapping_lidar_to_detection_frame_msg_).cast<float>();
    source_lidar_to_mapping_lidar_transform = tf2::transformToEigen(source_lidar_to_mapping_lidar_msg_).cast<float>();

  } catch (tf2::TransformException & ex) {
    RCLCPP_WARN(rclcpp::get_logger(name_), "Could not get initial tfs. Aborting filter. %s", ex.what());
    return calibration_frames;
  }
  ...
  std::sort(
    ...
    [](auto & lhs, auto & rhs) { return rclcpp::Time(lhs.header_.stamp) < rclcpp::Time(rhs.header_.stamp); });

  for (auto & calibration_frame : calibration_frames) {
    filter(calibration_frame, mapping_data, source_lidar_to_mapping_lidar_transform, mapping_lidar_to_detection_frame_transform);
  }
  ...
  const Eigen::Affine3f & mapping_lidar_to_detection_frame_transform)
{
  ...
  auto left_predicate = [&](const ObjectsBB & objects) {
    return rclcpp::Time(objects.header_.stamp) + rclcpp::Duration::from_seconds(parameters_->detection_max_time_tolerance_) > rclcpp::Time(calibration_frame.source_header_.stamp);
  };

  auto right_predicate = [&](const ObjectsBB & objects) {
    return rclcpp::Time(objects.header_.stamp) > rclcpp::Time(calibration_frame.source_header_.stamp) + rclcpp::Duration::from_seconds(parameters_->detection_max_time_tolerance_);
  };

  std::vector<ObjectsBB>::iterator left_it, right_it;

  left_it = std::find_if(mapping_data->detected_objects_.begin(), mapping_data->detected_objects_.end(), left_predicate);
  right_it = std::find_if(left_it, mapping_data->detected_objects_.end(), right_predicate);

  for (auto & it = left_it; it != right_it;  ++it ) {
    filter(calibration_frame, mapping_data, *it,
  ...
  const Eigen::Vector4f & min_p, const Eigen::Vector4f & max_p)
{
  ...
  const auto it = std::find_if(mapping_data->processed_frames_.begin(), mapping_data->processed_frames_.end(), predicate);
  ...
  Eigen::Matrix4f detections_mcs = poseInterpolation(
    (rclcpp::Time(objects.header_.stamp) - rclcpp::Time(left_frame->header_.stamp)).seconds(),
    0.f,
  ...
  Eigen::Affine3f source_to_detections_transform = source_lidar_to_mapping_lidar_transform * Eigen::Affine3f(calibration_frame.local_map_pose_.inverse()) * Eigen::Affine3f(detections_mcs) * mapping_lidar_to_detection_frame_transform;
...
void ObjectDetectionFilter::filter(
  ...
  const Eigen::Vector4f & min_p, const Eigen::Vector4f & max_p)
{\
  Eigen::Vector3f bbox_size = object.size_ + Eigen::Vector3f(parameters_->detection_size_tolerance_, parameters_->detection_size_tolerance_, parameters_->detection_size_tolerance_);
  ...
  Eigen::Affine3f source_to_object_transform = source_to_detections_transform * Eigen::Affine3f(object.pose_);

  auto no_intersection_predicate = [&source_to_object_transform, &min_p, &max_p](const Eigen::Vector3f & vertex){
    Eigen::Vector3f v = source_to_object_transform * vertex;
    return (v.x() < min_p.x() || v.x() > max_p.x()) && (v.y() < min_p.y() || v.y() > max_p.y()) && (v.z() < min_p.z() || v.z() > max_p.z());
  };
  ...
  boxFilter.setMin(Eigen::Vector4f(-0.5f*bbox_size.x(), -0.5f*bbox_size.y(), -0.5f*bbox_size.z(), 1.f));
  boxFilter.setMax(Eigen::Vector4f(0.5f*bbox_size.x(), 0.5f*bbox_size.y(), 0.5f*bbox_size.z(), 1.f));
