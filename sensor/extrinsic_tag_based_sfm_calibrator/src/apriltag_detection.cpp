...
void LidartagDetection::computeTemplateCorners(double new_size)
{
  assert(new_size > 0.0);
  size = new_size;
...
double ApriltagDetection::computeReprojectionError(double cx, double cy, double fx, double fy) const
{
  ...
  for (std::size_t corner_index = 0; corner_index < object_corners.size(); corner_index++) {
    ...
    double pr_x = cx + fx * (object_corner.x / object_corner.z);
    double pr_y = cy + fy * (object_corner.y / object_corner.z);
    double err_x = std::abs(pr_x - image_corner.x);
    double err_y = std::abs(pr_y - image_corner.y);
    error += std::sqrt(err_x * err_x + err_y * err_y);
  }
...
void ApriltagGridDetection::computeTemplateCorners(const TagParameters & tag_parameters)
{
  ...
  for (auto & sub_detection : sub_detections) {
    ...
    sub_detection.computeTemplateCorners(tag_parameters.size);
