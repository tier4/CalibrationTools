...
#ifndef EXTRINSIC_TAG_BASED_SFM_CALIBRATOR__CERES__CAMERA_RESIDUAL_HPP_
#define EXTRINSIC_TAG_BASED_SFM_CALIBRATOR__CERES__CAMERA_RESIDUAL_HPP_

#include <extrinsic_tag_based_sfm_calibrator/calibration_types.hpp>
#include <extrinsic_tag_based_sfm_calibrator/ceres/sensor_residual.hpp>
...
struct CameraResidual : public SensorResidual
{
  ...
  /*!
   * Auxiliar method to construct a 3d rotation matrix representing a 2d rotation matrix
   ...
   *   - the ground case case with a fixed shared component and the intrinsics not being optimized
   * @param[in] camera_pose_inv The pose from the camera to the origin
   * @param[in] tag_pose The pose of the tag. in the case of a ground tag, it refers to the 2d pose
   * @param[in] residuals The residual error of projecting the tag into the camera
   * @returns success status
   */
  template <typename T>
