...
class Plotter:
    def __init__(self):
        ...
        self.num_of_reflectors, self.cv_distance_error, self.cv_yaw_error, self.calibration_distance_error, self.calibration_yaw_error = 0, 0, 0, 0, 0
        self.prev_calibration_distance_error, self.prev_calibration_yaw_error, self.prev_cv_distance_error, self.prev_cv_yaw_error, self.prev_num_of_reflectors = 0, 0, 0, 0, 0
        self.cv_distance_error_list, self.cv_yaw_error_list, self.calibration_distance_error_list, self.calibration_yaw_error_list, self.num_of_reflectors_list = [0], [0], [0], [0], [0]
    ...
    def check_if_deleted(self):
        if self.num_of_reflectors < self.num_of_reflectors_list[-1]: 
            self.prev_cv_distance_error, self.prev_num_of_reflectors, index = self.redraw_subplot(self.num_of_reflectors, self.cv_distance_error_list ,self.subplot0, self.color_bo)
            self.prev_cv_yaw_error, _, _ = self.redraw_subplot(self.num_of_reflectors, self.cv_yaw_error_list ,self.subplot1, self.color_go)
            self.prev_calibration_distance_error, _, _ = self.redraw_subplot(self.num_of_reflectors, self.calibration_distance_error_list ,self.subplot2, self.color_bo)
            self.prev_calibration_yaw_error, _, _ = self.redraw_subplot(self.num_of_reflectors, self.calibration_yaw_error_list ,self.subplot3, self.color_go)
    ...
    def update_xy_lim(self):
        self.max_ylim0 = self.cv_distance_error if self.cv_distance_error > self.max_ylim0 else self.max_ylim0
        self.max_ylim1 = self.cv_yaw_error if self.cv_distance_error > self.max_ylim1 else self.max_ylim1
        self.max_ylim2 = self.calibration_distance_error if self.cv_distance_error > self.max_ylim2 else self.max_ylim2
        self.max_ylim3 = self.calibration_yaw_error if self.cv_distance_error > self.max_ylim3 else self.max_ylim3
    ...
    def draw_lines(self):
        #TODO(vivid):delete
        self.subplot0.plot([self.prev_num_of_reflectors, self.num_of_reflectors], [self.prev_cv_distance_error, self.cv_distance_error], self.color_bo)
        self.subplot1.plot([self.prev_num_of_reflectors, self.num_of_reflectors], [self.prev_cv_yaw_error, self.cv_yaw_error], self.color_go)
        self.subplot2.plot([self.prev_num_of_reflectors, self.num_of_reflectors], [self.prev_calibration_distance_error, self.calibration_distance_error], self.color_bo)
        self.subplot3.plot([self.prev_num_of_reflectors, self.num_of_reflectors], [self.prev_calibration_yaw_error, self.calibration_yaw_error], self.color_go)

        self.anno0 = self.subplot0.annotate('%0.4f' % self.cv_distance_error, xy=(self.num_of_reflectors, self.cv_distance_error), color=self.color_b)
        self.anno1 = self.subplot1.annotate('%0.4f' % self.cv_yaw_error, xy=(self.num_of_reflectors, self.cv_yaw_error), color=self.color_g)
        self.anno2 = self.subplot2.annotate('%0.4f' % self.calibration_distance_error, xy=(self.num_of_reflectors, self.calibration_distance_error), color=self.color_b)
        self.anno3 = self.subplot3.annotate('%0.4f' % self.calibration_yaw_error, xy=(self.num_of_reflectors, self.calibration_yaw_error), color=self.color_g)
        plt.pause(0.1)
